...
def parsePoint(line):
    ...
    Returns:
        LabeledPoint: The line is converted into a `LabeledPoint`, which consists of a label and
            features.
    """
    l = [float(x.encode('ascii','ignore')) for x in line.split(',')]
    return LabeledPoint(l[0],l[1:])

#lp = parsePoint(samplePoints[0])
#print "label"
#print lp.label
#print "features"
#print lp.features
...
# TODO: Replace <FILL IN> with appropriate code
averageTrainYear = (parsedTrainData.map(lambda x: x.label).reduce(lambda a,b:a+b)/parsedTrainData.count())
...
def calcRMSE(labelsAndPreds):
    ...
    """
    s = labelsAndPreds.map(lambda (x,y): squaredError(x,y)).sum()
    return math.sqrt(s/labelsAndPreds.count())
...
#print len(weightsLR0)
#print len(errorTrainLR0)
#print weightsLR0
#print errorTrainLR0
...
# TODO: Replace <FILL IN> with appropriate code
firstModel = LinearRegressionWithSGD. train(data=parsedTrainData, iterations=numIters, step=alpha, miniBatchFraction=miniBatchFrac, initialWeights=None, regParam=reg, regType=regType, intercept=useIntercept)
...
# TODO: Replace <FILL IN> with appropriate code
#print "parsedTrainData ",parsedTrainData.take(1)
...
labelsAndPreds = parsedTrainData.map(lambda x:(x.label, firstModel.predict(x.features)))
print labelsAndPreds.take(1)
...
alpha = 1.0
miniBatchFrac = 1.0
for reg in [1.0/10000000000.0,1.0/10000.0,1.0]:
...
print "bestRegParam = ", bestRegParam
print ('Validation RMSE:\n\tBaseline = {0:.3f}\n\tLR0 = {1:.3f}\n\tLR1 = {2:.3f}\n' +
...
for alpha in [1.0/100000.0, 10.0]:
...
def twoWayInteractions(lp):
    ...
    for x in lp.features:
        for y in lp.features:
            twoWay = twoWay + [x*y]
    return LabeledPoint(lp.label, list(lp.features) + list(twoWay))
